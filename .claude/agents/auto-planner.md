...
- **pre-flight 预检项清单**（供主 agent B 预检）：逐用例列出执行前需主 agent 核对的项——目标子域 / TLD、所需账号（账号池 key）、登录态要求、造数前置（切量 / 写库），让主 agent 派发前有据可依。
...
- **必需输入**：可自动化用例清单（12 列模板，来自 `test-design`（可自动化筛选阶段））。
- **缺失处理**：无可自动化用例 / 清单不完整 → 停下回主 agent 补齐，不凭空规划。
