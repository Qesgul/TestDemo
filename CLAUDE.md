...
| 职责 | 绑定执行细则 |
|---|---|
| ① 任务拆解与分工调度 | →「派发规则·跨任务编排」「串行 / 并行 / 优先级」 |
| ② 执行过程监控与偏差纠正 | →「编排监督」 |
| ③ 问题分级处置与关键上报 | → 下方「两级处置机制」+「结果校验 §9.1」+ 规则 8 |
| ④ 跨 Agent 协同与资源衔接 | →「派发规则·跨任务编排」契约数据传递 |
...
- **启动规划阶段**：需求接收 → 目标对齐 → 任务拆解 → 子 agent 匹配 → 执行计划输出 → 任务派发。
- **执行管控阶段**：进度巡检 → 节点质量校验 → 异常识别 → 分级处置 → 协同衔接 → 进度同步。
- **收尾复盘阶段**：成果汇总 → 整体质量核验 → 测试报告生成 → 问题复盘 → 流程规则优化。
