...
- **凭据必查账号池，禁止凭记忆**：需要登录账号时，统一从 `tests/data/account_pool.yaml`
  取账号/密码，**禁止凭记忆 / 沿用旧密码**；登录失败先核对账号池凭据，确认无误仍失败即判为
  账号锁定期（错密码多次会锁号约 15-30 分钟），**停手等解锁、禁止反复试登录**
  （详见 `.claude/playbooks/login-session.md`「账号锁定」）。
...
- [ ] 删除 / 覆盖文件前，是否已确认其真实用途与调用方？（防误删有用文件——本会话曾误删并未被 fixture 使用的登录缓存）
...
| 产品测试知识库（术语/业务规则/高频缺陷/启发式） | `.claude/knowledge/`（`test-design` 读取，`troubleshooter` 维护回写） |
...
- **C 缺陷复发第2次**：单次研发缺陷走规则8反馈用户、**不入库**；主 agent 对照 `recurring-risks`，同模式**第2次**出现才升级入 `recurring-risks`。
...
### 串行 / 并行 / 优先级
- **必须串行（有数据依赖）**：① `test-design` → ② `auto-case-extract` → ③ `auto-planner` →（**方案评审 gate**）→ ④ `code-engineer` → ⑤ `selector-debug` → ⑥ `test-runner`（执行验证）。
...
- **边界**：临时文件清理仍走 `cleanup`；本条专管数据 / 状态副作用还原；删除 / 还原既有数据前仍走规则 5。
